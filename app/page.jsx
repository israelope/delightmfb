        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 px-6 py-14 md:grid-cols-2 md:py-24">
          <div>
            ...
            <p className="mt-6 max-w-md font-body text-base leading-relaxed text-parchment-soft/80">
              Replacing the paper ledger with a transparent, always-current
              record — every contribution and every loan, verified by an admin
      ...
      <section id="products" className="border-b border-rule">
        <div className="mx-auto max-w-6xl px-6 py-14 text-center md:py-20">
          ...
          <p className="mx-auto mt-3 max-w-xl font-body text-ink-muted">
            A focused set of tools that replace the paper ledger, nothing more,
            nothing less.
          </p>
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 text-left sm:grid-cols-3">
            {PRODUCTS.map(({ icon: Icon, title, body }) => (
              <div
                key={title}
                className="rounded-sm border border-rule bg-parchment-soft p-6"
              >
                <div className="flex h-11 w-11 items-center justify-center rounded-sm bg-cooperative/10">
                  <Icon
                    className="h-5 w-5 text-cooperative"
                    strokeWidth={1.75}
                  />
                </div>
            ...
            <div>
              ...
              <ul className="mt-5 space-y-2.5">
                {[
                  ...
                  <li
                    key={item}
                    className="flex items-start gap-2.5 font-body text-sm text-ink"
                  >
                    <ShieldCheck
                      className="mt-0.5 h-4 w-4 shrink-0 text-cooperative"
                      strokeWidth={2}
                    />
                    {item}
                  </li>
      ...
      <section className="border-b border-rule bg-parchment-soft">
        <div className="mx-auto max-w-6xl px-6 py-14 md:py-20">
          ...
          <p className="mt-2 max-w-xl font-body text-ink-muted">
            Four steps, the same order every officer has used offline for years
            — now just faster to verify.
          </p>
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 text-left sm:grid-cols-3">
            {TESTIMONIALS.map(({ quote, name, since }) => (
              <div
                ...
                className="rounded-sm border border-rule bg-parchment-soft p-6"
              >
                <p className="font-body text-sm italic leading-relaxed text-ink">
                  &ldquo;{quote}&rdquo;
                </p>
          ...
          <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-4">
            <div>
              ...
              <p className="mt-3 font-body text-sm text-parchment-soft/70">
                A digital ledger for our Thrift &amp; Credit Cooperative Society
                — replacing paper passbooks with a transparent, admin-verified
                record.
              </p>
            </div>

            <div>
              <p className="font-body text-sm font-semibold text-parchment-soft">
                Quick Links
              </p>
              <ul className="mt-3 space-y-2 font-body text-sm text-parchment-soft/70">
                <li>
                  <a href="#home" className="hover:text-brass-light">
                    Home
                  </a>
                </li>
                <li>
                  <a href="#about" className="hover:text-brass-light">
                    About
                  </a>
                </li>
                <li>
                  <a href="#products" className="hover:text-brass-light">
                    Products
                  </a>
                </li>
                <li>
                  <Link href="/register" className="hover:text-brass-light">
                    Register
                  </Link>
                </li>
                <li>
